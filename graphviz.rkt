...
#|
Entiendo que meramente lo que se cambiaron fueron los atributos esteticos y 
de formato del archivo .dot, para que se vea mas bonito y legible, con nodos redondos y bordes mas limpios
para los estados normales, formas de doble círculo para los estados finales, 
...
;-----------CREAR NODO DOT------------------
;Metemos formas redondas y bordes mas limpios para los estados normales, y formas de doble círculo para los estados finales, y un estilo destacado para el estado inicial.
...
(define (state->dot state start-state finales)
  ...
  (cond
    ...
    [else
     (string-append "  \"" state "\" [shape=circle, style=filled, fillcolor=\"#F5F5F5\", color=\"#757575\", penwidth=1.5, fontname=\"Helvetica\"];\n")]))
...
(define (edges->dot grouped-edges)
  (foldl
   (lambda (key accum)
     ...
     (string-append
      accum
      "  \"" from-state "\" -> \"" to-state "\" [label=\"  " label " \", fontname=\"Helvetica\", fontsize=10, color=\"#555555\", penwidth=1.2];\n"))
   ""
...
(define (get-dot-executable)
  (cond
    ...
    [else (error 'graphviz "No se encontró el comando 'dot' de Graphviz instalado en el sistema")]))
...
(define (generate-graphviz automaton dot-path png-path)
  ...
  (system (format "\"~a\" -Tpng \"~a\" -o \"~a\"" dot-exe dot-path png-path))
  (void))