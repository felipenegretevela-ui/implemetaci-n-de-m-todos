...
(provide generate-graphviz automaton->dot collect-edges)
...
(define (format-edge-label joined-syms pop push dir)
  (define (clean str) (if (or (equal? (string-trim str) "") (equal? (string-trim str) "ε")) "ε" (string-trim str)))
  (match (list (clean pop) (clean push) (string-trim dir))
    [(list "ε" "ε" "") joined-syms]
    [(list p-pop p-push "") (format "~a, ~a → ~a" joined-syms p-pop p-push)]
    [(list _ p-push d) (format "~a → ~a, ~a" joined-syms p-push d)]))

(define (collect-edges automaton)
  (for/fold ([out ""]) ([state (hash-ref automaton "estados")])
    (define grouped-edges (make-hash))
    
    ; Agrupar flechas que van al mismo estado con las mismas instrucciones
    (hash-for-each (hash-ref automaton state (hash))
                   (lambda (sym dests)
                     (for-each (lambda (dest-tuple)
                                 (hash-update! grouped-edges dest-tuple (lambda (syms) (cons sym syms)) '()))
                               dests)))
      ...
      (for/fold ([acc ""]) ([(dest-tuple syms) (in-hash grouped-edges)])
        (match dest-tuple
          [(list to-state pop push dir)
           (define joined-syms (string-join (sort (remove-duplicates (map (lambda (s) (if (equal? s "") "ε" s)) syms)) string<?) ","))
           (string-append acc (format "  \"~a\" -> \"~a\" [label=\"~a\"];\n" state to-state (format-edge-label joined-syms pop push dir)))])))
...
(define (automaton->dot automaton)
  ...
  (define nodes
    (string-join (map (lambda (s) (state->dot s inicial (hash-ref automaton "finales"))) (hash-ref automaton "estados")) ""))
...
(define (generate-graphviz automaton dot-path png-path)
  (parameterize ([current-directory here])
    (call-with-output-file dot-path #:exists 'replace (lambda (out) (display (automaton->dot automaton) out)))
    (system (format "dot -Tpng \"~a\" -o \"~a\"" dot-path png-path))))