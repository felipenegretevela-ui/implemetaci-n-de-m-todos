...
;-----------COMPACT HTML RENDERING FUNCTION------------------
(define (render-page current-code html-code html-checks img-base64)
  (response/full
...
<head>
  ...
  <style>
    ...
    textarea { width: 100%; height: 280px; background: #1E1E1E; color: #fff; border: 1px solid #3A3A3A; padding: 15px; font-family: monospace; font-size: 14px; border-radius: 6px; box-sizing: border-box; }
    button { background: #fff; color: #000; border: none; padding: 10px 25px; font-size: 16px; cursor: pointer; border-radius: 4px; font-weight: bold; margin-top: 10px; }
...
(define (start request)
  ...
  (if (exists-binding? 'codigo bindings)
      (let* ([code (string-replace (extract-binding/single 'codigo bindings) "\r\n" "\n")]
             [automaton (parse-start (tokenizer code tokens-table))]
             ...
             [tokens-raw (tokens->html (tokenizer code tokens-table))]
        ...
        (render-page code tokens-final (style-validations (validate-checks automaton)) img-b64))
      
      (render-page default-code #f #f "")))
